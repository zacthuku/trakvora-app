...
export default function MarketplaceScreen() {
  ...
  const { data: loads, isLoading, refetch } = useQuery({
    queryKey: ["owner-marketplace", selectedCargo],
    queryFn: () => ownerApi.getMarketplace(selectedCargo ? { cargo_type: selectedCargo } : {}),
  });
  const onRefresh = async () => { setRefreshing(true); await refetch(); setRefreshing(false); };

  const bidMut = useMutation({
    mutationFn: ({ loadId, amount }: { loadId: number; amount: number }) =>
      ownerApi.placeBid({ load_id: loadId, amount }),
    onSuccess: () => { Alert.alert("Bid placed!"); qc.invalidateQueries({ queryKey: ["owner-bids"] }); },
  ...
  return (
    <View style={styles.screen}>
      <Text style={styles.title}>Load Marketplace</Text>
      <FlatList
        horizontal
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={[styles.chip, selectedCargo === item.value && styles.chipActive]} onPress={() => setSelectedCargo(item.value)}>
        ...
        renderItem={({ item: load }) => (
          <View style={styles.card}>
            ...
            <View style={styles.bidRow}>
              ...
              />
              <Button size="sm" onPress={() => bidMut.mutate({ loadId: load.id, amount: Number(bidAmounts[load.id]) })} disabled={!bidAmounts[load.id]} loading={bidMut.isPending}>Bid</Button>
            </View>
          </View>
        )}
        ListEmptyComponent={<Text style={styles.empty}>No loads available.</Text>}
      />
    </View>
  );
}

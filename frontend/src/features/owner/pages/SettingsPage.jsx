  ...
  return (
    <label className="flex items-start gap-4 cursor-pointer py-3 group">
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-slate-800 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{label}</p>
        {sub && <p className="text-xs text-slate-400 mt-0.5">{sub}</p>}
      </div>
      <div
        onClick={onChange}
        className={`relative w-11 h-6 rounded-full shrink-0 mt-0.5 transition-colors cursor-pointer ${checked ? "bg-secondary" : "bg-slate-200 dark:bg-slate-600"}`}
      >
        <span
...
const KYC_STATUS_META = {
  unverified: { label: "Unverified",    color: "bg-slate-100 text-slate-600 border-slate-200" },
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Required to receive payouts and manage your fleet.</p>
        </div>
        ...
        <form onSubmit={(e) => { e.preventDefault(); kycMut.mutate({ id_type: idType, id_number: idNumber, ...(selfieUrl ? { selfie_url: selfieUrl } : {}) }); }}
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
              Selfie <span className="text-slate-400 font-normal">(optional — helps match face to ID)</span>
            </label>
            <div className="flex items-center gap-3">
              {selfieUrl ? (
                <img src={selfieUrl} alt="selfie" className="w-16 h-16 rounded-xl object-cover border-2 border-teal-300 shrink-0" />
              ) : (
                <div className="w-16 h-16 rounded-xl bg-slate-100 dark:bg-slate-700 border-2 border-dashed border-slate-300 dark:border-slate-500 flex items-center justify-center shrink-0">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">ID Type</label>
            <select value={idType} onChange={(e) => setIdType(e.target.value)} className={inputCls}>
              {ID_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">ID Number</label>
...
export default function SettingsPage() {
  ...
  const [notifs, setNotifs] = useState(() => {
    const defaults = {
      load_claimed: true, truck_assigned: true, payment_received: true,
      document_expiry: true, system_updates: false, sms_alerts: false, email_digest: true,
    };
    try {
      const stored = localStorage.getItem(`trakvora_notif_${user?.id}_owner`);
  ...
  const handleNotifSave = () => {
    localStorage.setItem(`trakvora_notif_${user?.id}_owner`, JSON.stringify(notifs));
    setNotifSaved(true);
  ...
  return (
    <div className="w-full max-w-3xl">
      ...
      {/* ── Profile section ── */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-6 mb-5">
        <SectionHeader icon={User} title="Profile Information" sub="Update your personal and company details." />

        {/* Avatar */}
        <div className="flex items-center gap-5 mb-6">
          <ProfilePhotoPicker user={user} initials={initials} size="lg" onUpdated={handlePhotoUpdated} />
          <div>
        ...
        <form onSubmit={handleProfileSave} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1.5 uppercase tracking-wider">Full Name</label>
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Email cannot be changed. Contact support if you need to update it.</p>
          </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-6 mb-5">
        ...
        {/* Account info */}
        <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-700 grid grid-cols-2 gap-4">
          <div>
      ...
      {/* ── Role change ── */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm px-6 py-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Change how you use Trakvora. Your history is always preserved.</p>
          </div>
        </div>
        <RoleChangeCard />
      </div>
      ...
      {/* ── Danger zone ── */}
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-red-200 dark:border-red-900/50 shadow-sm px-6 py-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Account actions. Proceed with caution.</p>
          </div>
        </div>

    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="text-2xl font-heading font-bold text-white">Parcel Bookings</h1>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-800">
            ...
            {items.map((item) => (
              <tr key={item.id} className="hover:bg-slate-800/40 transition-colors">
                <td className="px-4 py-3 max-w-[220px]">
                  <p className="truncate text-white text-xs font-medium">{item.pickup_location}</p>
                  <p className="truncate text-slate-500 text-xs">→ {item.dropoff_location}</p>
                </td>
                <td className="px-4 py-3 text-slate-300">{item.weight_kg} kg</td>
                <td className="px-4 py-3 capitalize text-slate-300">{item.service_level?.replace("_", " ")}</td>
            ...
            {!isLoading && items.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-500">No parcel bookings yet</td></tr>
            )}
          </tbody>
        </table>
      </div>
